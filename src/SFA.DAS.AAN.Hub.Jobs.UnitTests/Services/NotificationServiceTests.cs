...
public class NotificationServiceTests
{
    ...
    public async Task Init()
    {
        ...
        _messagingSessionMock.Setup(m => m.Send(It.Is<SendEmailCommand>(c => c.RecipientsAddress == _notifications.First().Member.Email), It.IsAny<SendOptions>(), It.IsAny<CancellationToken>())).Throws<Exception>();
    ...
    public void ThenSendsMessageForEachNotification() =>
        _messagingSessionMock.Verify(m => m.Send(It.IsAny<SendEmailCommand>(), It.IsAny<SendOptions>(), It.IsAny<CancellationToken>()), Times.Exactly(_notifications.Count));
    ...
    public void ThenConvertsNotificationToSendEmailCommand()
    {
        ...
        _messagingSessionMock.Verify(m => m.Send(It.Is<SendEmailCommand>(c => c.TemplateId == templateId && c.RecipientsAddress == notification.Member.Email), It.IsAny<SendOptions>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public void ThenAddsLinkTokenBasedOnMemberUserType()
    {
        ...
        _messagingSessionMock.Verify(m => m.Send(It.Is<SendEmailCommand>(c => c.TemplateId == templateId && c.Tokens[NotificationService.LinkTokenKey].Contains(uri.ToString())), It.IsAny<SendOptions>(), It.IsAny<CancellationToken>()));
    }
}
