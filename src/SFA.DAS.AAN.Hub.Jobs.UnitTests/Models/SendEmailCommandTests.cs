...
public class SendEmailCommandTests
{
    ...
    public void Init()
    {
        ...
        _notification = fixture
            ...
            .With(n => n.Member, fixture.Build<Member>().With(m => m.UserType, SendEmailCommand.UserTypeApprentice).Create())
            .Create();
    ...
    public void ThenPopulatesTokensFromNotificationTokens()
    {
        ...
        _sut.Tokens.Should().HaveCount(2 + 1);
        _sut.Tokens[nameof(TestTokens.Id)].Should().Be(_testTokens.Id);
        _sut.Tokens[nameof(TestTokens.Name)].Should().Be(_testTokens.Name);
    }

    [Test]
    public void ThenAddsLinkToken()
    {
        _sut.Tokens[SendEmailCommand.LinkTokenKey].Should().Be(new Uri(_applicationConfiguration.ApprenticeAanRouteUrl, $"links/{_notification.Id}").ToString());
    }
}
